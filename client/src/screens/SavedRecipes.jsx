...
const SavedRecipes = () => {
    ...
    const getRecipes = async () => {
        setLoading(true);
        try {
            const res = await axios.get(`${import.meta.env.VITE_API_KEY}/recipes/savedRecipes/${userInfo.userID}`,
                {
        ...
        <div style={{ padding: "20px" }}>
            ...
            <Grid container spacing={3}>
                {loading ?
                    ...
                    <>
                        {recipes?.filter((v) => !keyword ? v : v.name.toLowerCase().includes(keyword.toLowerCase()))
                            .map((v) => {
                                return (
                                    <Grid
                                        size={{ lg: 3, md: 6, sm: 12, xs: 12 }}
